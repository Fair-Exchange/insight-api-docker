...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  transformed.valueOut = (transaction.outputSatoshis / 1e8).toFixed(8) ;
  transformed.size = transaction.hex.length / 2; // in bytes
  if (!transaction.coinbase) {
    transformed.valueIn = (transaction.inputSatoshis / 1e8).toFixed(8);
    transformed.fees = (transaction.feeSatoshis / 1e8).toFixed(8);
  }
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  transformed.value = (input.satoshis / 1e8).toFixed(8);
...
TxController.prototype.transformInvTransaction = function(transaction) {
  ...
  var transformed = {
    txid: transaction.hash,
    valueOut: (valueOut / 1e8).toFixed(8),
    vout: vout,
    isRBF: isRBF,
  };
